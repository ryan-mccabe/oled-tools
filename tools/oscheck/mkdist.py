...
# questions.
"""
Create a zipapp distribution of oscheck which can be executed directly
"""
import argparse
import shutil
import tempfile
...
def main():
    parser = argparse.ArgumentParser(
        description="create oscheck distributions"
    )
    parser.add_argument(
        "--interpreter",
    ...
    base_dir = Path(__file__).parent / "oscheck"
    entry_point = "oscheck.__main__:main"
    ...
    with tempfile.TemporaryDirectory() as td:
        tmp = Path(td)
        shutil.copytree(base_dir, tmp / "oscheck")
